...
use std::error::Error;
use tokio; // Import Tokio for the runtime
...
impl StockScraper {
    ...
    pub async fn scrape(&self, exchange: &str, ticker: &str) -> Result<HashMap<String, String>, Box<dyn Error>> {
        let target_url = format!("{}/quote/{}:{}?hl={}", self.base_url, ticker, exchange, self.language);
